function smartHouse(JsonDataIn) {
  ...
  for (let i = 0; i < rates.length; i++) {
    ...
    //Ночь с 21 до 7
    if (start >= 0 && start < 7 || start >= 21 && start < 24) ratesNArr.push(rates[i]);  // Заполняем массив ночных тарифов

    //День с 7 до 21
    if (start >= 7 && start < 21) ratesDArr.push(rates[i]);  // Заполняем массив дневных тарифов
  }
      ...
      if (!(undBool) && (currRate.from === 7 || currRate.from === 21 || currRate.to === 7 || currRate.to === 21)) {
        ...
        if (currRate.from === 7 || currRate.from === 21) {
          ...
          if (ratesArr[searchIdx].to === rateTo) {
            ...
            return searchIdx;  // Возвращаем индекс найденного тарифа
          }
          ...
          for (let key in JsonDummyOut.schedule) {
            ...
            if (key == ratesArr[searchIdx].from) {  // Находим совпадение времени в рассписании со временем начала следующего тарифа
              ...
              if (ratesArr[searchIdx].to > ratesArr[searchIdx].from) {  // Случай без перехода через 0
                ...
                for (let i = 0; i < ratesArr[searchIdx].to - ratesArr[searchIdx].from + 24; i++) {

                  if (parseInt(key) + i < 24) {  // Случай до перехода через 0

                    JsonDummyOut.schedule[parseInt(key) + i].forEach((item) => {
          ...
          while(true) {
            ...
            if (ratesArr[searchIdx].to === currRate.from) {
              ...
              for (let key in JsonDummyOut.schedule) {
                ...
                if (key == ratesArr[searchIdx].to) {

                  if (ratesArr[searchIdx].to > ratesArr[searchIdx].from) {

                    for (let i = 0; i < ratesArr[searchIdx].to - ratesArr[searchIdx].from; i++) {  // Случай без перехода через 0
  ...
  function rateCalc(devItem, ratesValue) {
    ...
    for (let key in JsonDummyOut.consumedEnergy.devices) {
      ...
      if (key === devId) result = true;

    }

    // Добавляем устройство в список если отсутствует
    if (!(result)) JsonDummyOut.consumedEnergy.devices[devId] = 0;
  ...
  devDArr.forEach((item, idx) => {
    ...
    dPowArr.forEach((item, idx) => {
      ...
      devPow <= maxPow - item && idx >= 7 && idx < 21 ? freeCells++ : freeCells = 0;  // Подсчитываем свободные ячейки времени
      
      if (freeCells > maxCells) maxCells = freeCells;  // Записываем если свободно больше текущего максимума
    });
  ...
  devNArr.forEach((item, idx) => {
    ...
    dPowArr.forEach((item, idx) => {
      ...
      fromZero && devPow <= maxPow - item && idx < 7 ? addCells++ : fromZero = false;   // Подсчитываем свободные ячейки от 0 до первой несвободной или до 7

      if (idx === 23 && freeCells !== 0) freeCells += addCells;  // Добавляем свободные ячейки от нуля к ячейкам в конце если на 23 часа существуют свободные ячейки

      if (maxCells < freeCells) maxCells = freeCells;  // Записываем если свободно больше текущего максимума
    });
      ...
      if (rates[ratesIdx].from < rates[ratesIdx].to) {
        ...
        // Если текущий час на границе двух тарифов
          ...
          if (searchIterations > 14) {
            ...
            // Проверка на наличие следующего тарифа в массиве ночных тарифов
            if (searchPlus && rates[ratesIdx + 1]) {
        ...
        if (rates[ratesIdx].from + timeAssigned + timePassed < 24) {
          ...
          if (!(zeroPass) && dPowArr[rates[ratesIdx].from + timeAssigned + timePassed] + devPow <= maxPow) {
            ...
            JsonDummyOut.schedule[rates[ratesIdx].from + timeAssigned + timePassed].push(devId);
            dPowArr[rates[ratesIdx].from + timeAssigned + timePassed] += devPow;

            rateCalc(item, rates[ratesIdx].value);
            timeAssigned++;
            ...
            if (searchIterations > 14) {
              ...
              ratesIdx = searchEngine(rates, ratesIdx, item);
              searchIterations++;
            }
          ...
          if (dPowArr[timeAssigned + timePassed] + devPow <= maxPow && timeAssigned + timePassed < rates[ratesIdx].to) {
            ...
            JsonDummyOut.schedule[timeAssigned + timePassed].push(devId);
            dPowArr[timeAssigned + timePassed] += devPow;
  ...
  devUArr.forEach((item, idx) => {
    ...
    dPowArr.forEach((item, idx) => {
      ...
      devPow <= maxPow - item ? freeCells++ : freeCells = 0;  // Подсчитываем свободные ячейки времени

      fromZero && devPow <= maxPow - item ? addCells++ : fromZero = false;  // Подсчитываем свободные ячейки от 0 до первой несвободной

      if (idx === 23 && freeCells !== 0 && freeCells !== 24) freeCells += addCells;  // Добавляем свободные ячейки от 0 к ячейкам в конце если на 23 часа существуют свободные ячейки и их кол-во не 24
      ...
      if (rates[ratesIdx].from < rates[ratesIdx].to) {
        ...
        // Если текущий час на границе двух тарифов
          ...
          if (item.duration === 24 || searchIterations > 24) {
            ...
            // Проверка на наличие следующего тарифа в массиве круглосуточных тарифов
            if (searchPlus && rates[ratesIdx + 1]) {
        ...
        if (rates[ratesIdx].from + timeAssigned + timePassed < 24) {
          ...
          if (!(zeroPass) && dPowArr[rates[ratesIdx].from + timeAssigned + timePassed] + devPow <= maxPow) {
            ...
            if (item.duration === 24 || searchIterations > 24) {
              ...
              ratesIdx = searchEngine(rates, ratesIdx, item, true);
              searchIterations++;
            }
